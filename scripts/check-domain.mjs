...
if (!res) {
  ...
  if (robots?.includes('/welcome')) pass('robots.txt', 'serves and allows /welcome')
  else fail('robots.txt', 'missing or does not allow /welcome')

  const appPage = await safe(() => fetch(`https://${domain}/my-day`, { redirect: 'manual' }))
  if (!appPage) {
    warn('/my-day', 'could not test')
  } else if (appPage.status >= 300 && appPage.status < 400) {
